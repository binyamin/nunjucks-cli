...
- Detect color-support within the **log** filter
  ([`52a93a9`](https://github.com/binyamin/nunjucks-cli/commit/52a93a9))
- In the **log** filter, print the primitive _value_ of a object-wrapped (boxed)
  primitive.
  ([`52a93a9`](https://github.com/binyamin/nunjucks-cli/commit/52a93a9)).

  Meaning, `Number('2')` returns the numeric value "2". In contrast,
  `new Number('2')` returns a
  [`Number` object](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/Number)
  which _contains_ the number "2". To access the number, we need to use the
  `.valueOf()` function.
