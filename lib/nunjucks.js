...
export async function createEnv({ paths, dataFiles=[] }) {
    ...
    // Functions
    for(const funcName in plugins.functions) {
        env.addGlobal(funcName, plugins.functions[funcName]);
    }

    // Filters
    for(const filterName in plugins.filters) {
        const filter = plugins.filters[filterName];
        env.addFilter(filterName, filter, isAsyncFunction(filter));
    }
