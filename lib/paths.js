 ...
 * @prop {string|undefined} src relative path to dir
 * containing {@link PathConfig.files}
 * @prop {string} dest relative path to dir for output files.
 * Replaces {@link PathConfig.src}
 * @prop {string[]} files list of paths to input files; relative
 * to {@link PathConfig.src}
 * @prop {string} [outputFile] path for output file; relative
 * to {@link PathConfig.dest}. Only exists if the user
 * passes a file for the output argument
 */
...
export async function getPaths(rawSource, rawOutput) {
	...
	// [2] `PathConfig.src`
	// We need to know whether `inputArgument` is a file or a directory. If
	// it's a file, then `PathConfig.src` should equal `undefined`
	PathConfig.src = isFile(inputArgument) ? undefined : inputArgument;
	...
	if (isFile(outputArgument)) {
		...
		// `PathConfig.outputFile` to the filename and `PathConfig.dest` to the
		// directory path.
	...
	if (PathConfig.src) {
		...
		const temporaryFiles = await walk(PathConfig.src);
		PathConfig.files = temporaryFiles.map(file => (
			path.relative(path.resolve(PathConfig.src), file)
		)).filter(file => /\.(njk|html)$/.test(file));
	} else {
